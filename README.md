...
back to `kicad-cli` defaults, and the report says which source each option came from. Nothing in
your project is ever deleted or modified; files already in a board's folder are left alone and
kept out of the zip.
...
2. Copy the `plugin` folder into KiCad's user plugins folder, for example
...
3. Restart KiCad. The PCB editor toolbar gets **Export Boards** and **Export Boards (Dry Run)**.
