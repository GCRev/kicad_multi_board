...
def check_version(version: Optional[tuple[int, int]], generator_version: Optional[str]) -> Optional[str]:
    """An error message if kicad-cli cannot be trusted to read this board, else None."""
    if version is None:
        return "could not determine the kicad-cli version (is the path correct?)"
    match = re.match(r"(\d+)\.(\d+)", generator_version or "")
    if not match:
        return None  # the board does not say which version wrote it; nothing to compare
    board = (int(match.group(1)), int(match.group(2)))
    if board != version:
        return (f"kicad-cli is version {version[0]}.{version[1]} but the board was written by "
                f"{board[0]}.{board[1]}; it may not read this board correctly")
    return None
